...
import type { IPost, IPostPopulated } from "@/models/Post";

const PostTable = ({ posts, renderActions, linkBasePath = "/dashboard/posts", linkField = "_id" }:{posts: IPostPopulated[]; renderActions?: (post: IPostPopulated) => React.ReactNode; linkBasePath?: string; linkField?: keyof IPost;}) => {
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {sortedPosts.map((post) => (
              <tr key={post._id.toString()} className="group hover:bg-surface-muted transition-colors">
